...
int main(void)
{
	...
	/*
	cvThreshold(gray_img, gray_img2, 140, 255, CV_THRESH_BINARY);

	cvCvtColor(gray_img2, dst_img, CV_GRAY2BGR);
	for(y = 0; y < dst_img->height; y++) {
		for(x = 0; x < dst_img->width; x++) {
			if(dst_img->imageData[dst_img->widthStep * y + x * 3 + 0] == 0) {
